...
public:
    ...
    int dfs(int node, int parent, unordered_map<int, vector<int>>& adj, string& s) {
        ...
        for (auto& it : adj[node]) {
            ...
            if (longestpathfromnode > longest) {
                second_longest = longest;  // Demote longest to second_longest
                longest = longestpathfromnode;
            } else if (longestpathfromnode > second_longest) {
